...
public import Erdos1201.Vendor.NumberTheory.FriableIntegers.PrimeBandQuadrature

/-!
# Oscillatory prime sums under the medium-strength Prime Number Theorem

Abel summation transfers the medium-strength PNT error term for `θ` (the
first Chebyshev function) to an explicit bound on the oscillatory weighted
prime sum `∑ p, cos(τ log p) / p` over primes in a dyadic-type block,
uniformly in the frequency `τ ≠ 0`.
-/
...
namespace PrimeCosineOscillationBound

open FriableIntegers.PrimeSums
open FriableIntegers.PrimeBandQuadrature
...
theorem hasDerivAt_oscKernel (tau : ℝ) {x : ℝ} (hx : x ≠ 0) :
  ...
  all_goals first | rfl | (simp only [id_eq, Function.comp_apply]; field_simp; ring)
...
theorem invLog_mul_blockWeightedCoeff_sum_eq_primeOscillation
  ...
  intro n hn
  have hn' := Finset.mem_Ioc.mp hn
  rw [blockWeightedCoeff, ite_eq_left hn'.1, primeLogCoeff]
  by_cases hp : n.Prime
...
theorem hasDerivAt_invLog {x : ℝ} (hx : x ≠ 0) (hlog : Real.log x ≠ 0) :
  ...
  obtain ⟨_, hx1, hxm1⟩ := Real.log_ne_zero.mp hlog
  exact Real.hasDerivAt_inv_log hx hx1 hxm1

theorem deriv_invLog {x : ℝ} (_hx : x ≠ 0) (_hlog : Real.log x ≠ 0) :
...
theorem exists_mediumThetaError :
  ...
  have htri : |thetaError x| ≤
    ...
    all_goals first | ring_nf | congr 1
...
/-- The medium-strength PNT gives an explicit `A`-uniform bound, valid for
`τ ≠ 0`, on the oscillatory prime sum `∑ cos(τ log p) / p` over a block
`(A, B]` starting past some absolute threshold `X₀`. -/
...
end PrimeCosineOscillationBound
