...
namespace PrimePowerLogMassLinearBound

/-- A deliberately generous but completely explicit dyadic estimate. -/
theorem dyadic_weight_sum_eq (n : ℕ) :
...
/-- The prime-power logarithmic mass used in the Halberstam--Richert
proof. -/
noncomputable def pairPrimePowerMass (h : ℕ → ℝ) (Q : ℕ) : ℝ :=
  ∑ pj ∈ primePowerPairs Q,
    h (pj.1 ^ (pj.2 + 1)) * Real.log ((pj.1 ^ (pj.2 + 1) : ℕ) : ℝ)

/-- The mass used by `PrimePowerLogConvolution.primePowerMass`: the prime
`p` runs below `Q+1` and the positive exponent runs through
`1,...,log_p Q`. -/
...
theorem primePowerPairs_zero : primePowerPairs 0 = ∅ := by
  simp [primePowerPairs]

theorem pairPrimePowerMass_zero (h : ℕ → ℝ) : pairPrimePowerMass h 0 = 0 := by
  simp [pairPrimePowerMass, primePowerPairs]

theorem sum_Icc_one_eq_sum_range_succ (f : ℕ → ℝ) (L : ℕ) :
...
theorem primesBelow_succ_eq_primesLE (Q : ℕ) :
    (Q + 1).primesBelow = Nat.primesLE Q := by
  ext p
  simp [Nat.mem_primesBelow, Nat.mem_primesLE]

theorem weighted_geometric_tail_le
  ...
  have hnat := hasSum_coe_mul_geometric_of_norm_lt_one hrnorm
  have hgeom := hasSum_geometric_of_lt_one hr0 hr1
  have hsum : HasSum (fun k : ℕ => ((k + 2 : ℕ) : ℝ) * r ^ k)
      (r / (1 - r) ^ 2 + 2 * (1 - r)⁻¹) := by
    have heq : (fun k : ℕ => ((k + 2 : ℕ) : ℝ) * r ^ k) =
...
theorem tail_prime_power_term_le
...
finite encoding used by `PrimePowerLogConvolution.primePowerMass`. -/
...
end PrimePowerLogMassLinearBound
