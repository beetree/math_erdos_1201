...
namespace PrimePowerLogConvolution

/-- The logarithmically weighted partial sum of an arithmetic weight. -/
noncomputable def logPartialSum (h : ℕ → ℝ) (N : ℕ) : ℝ :=
  ∑ n ∈ Finset.Icc 1 N, h n * Real.log (n : ℝ)

/-- The logarithmic prime-power mass of an arithmetic weight below `Q`. -/
noncomputable def primePowerMass (h : ℕ → ℝ) (Q : ℕ) : ℝ :=
  ∑ p ∈ (Q + 1).primesBelow,
    ∑ nu ∈ Finset.Icc 1 (Nat.log p Q),
      h (p ^ nu) * Real.log ((p ^ nu : ℕ) : ℝ)

theorem log_eq_sum_primeFactors (n : ℕ) :
    Real.log (n : ℝ) =
      ∑ p ∈ n.primeFactors,
...
theorem weighted_log_eq_sum_primeFactors
    (h : ℕ → ℝ)
...
theorem sourceToTarget_injective_on (N : ℕ) :
...
theorem sourceToTarget_mem_targetSet {N : ℕ} {a : SourceIndex}
...
theorem sourceWeight_eq_targetWeight_sourceToTarget
    (h : ℕ → ℝ) (a : SourceIndex) :
    sourceWeight h a = targetWeight h (sourceToTarget a) := by
  rfl

theorem targetWeight_nonneg
...
/-- The exact logarithmic prime-power convolution identity behind the
Halberstam--Richert log-moment estimate: the log-weighted partial sum is
bounded by the divisor convolution of `h` against the prime-power
logarithmic mass. -/
