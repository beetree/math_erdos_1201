...
mean-value estimate.  The theorem proved here is the Rankin/Euler-product
bound with exact constant `1`.  Its missing factor `1 / log x` is precisely
the genuinely sieve-theoretic part of the classical Halberstam--Richert
lemma.
-/
...
namespace HalberstamRichertMeanValue
...
/-- Exact local Euler-factor estimate implied by a linear prime-power decay
hypothesis on `h`. -/
...
theorem explicitMassConstant_nonneg {lambda1 lambda2 : ℝ}
    (h1 : 0 ≤ lambda1) (h2 : 0 ≤ lambda2) :
    0 ≤ explicitMassConstant lambda1 lambda2 := by
  unfold explicitMassConstant
  positivity

theorem recipWeight_one {h : ℕ → ℝ} (h1 : h 1 = 1) :
    recipWeight h 1 = 1 := by
  simp [recipWeight, h1]

theorem recipWeight_mul {h : ℕ → ℝ} (h0 : h 0 = 0)
...
theorem nat_Icc_mem_smoothNumbers {x n : ℕ}
...
provides `hlog`, the constant is exactly `K + 1`.
-/
...
end HalberstamRichertMeanValue
