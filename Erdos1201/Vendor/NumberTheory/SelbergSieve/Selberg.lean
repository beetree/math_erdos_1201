...
public import Erdos1201.Vendor.NumberTheory.SelbergSieve.Basic
...
open Finset Real Nat SelbergSieve.P520.UpperBoundSieve ArithmeticFunction SelbergSieve BoundingSieve
...
local notation3 "g" => SelbergSieve.P520.selbergTerms (SelbergSieve.toBoundingSieve (self := s))
...
set_option quotPrecheck false
local notation3 "S" => SelbergSieve.P520.selbergBoundingSum s
...
theorem selbergBoundingSum_ne_zero : S ≠ 0 := by
  ...
  exact SelbergSieve.P520.selbergBoundingSum_pos s

theorem selbergBoundingSum_nonneg : 0 ≤ S := _root_.le_of_lt (SelbergSieve.P520.selbergBoundingSum_pos s)
...
set_option quotPrecheck false
local notation3 "γ" => SelbergSieve.P520.selbergWeights s
...
theorem selbergWeights_mul_mu_nonneg (d : ℕ) (hdP : d ∣ P) :
  ...
  · exact SelbergSieve.P520.selbergBoundingSum_nonneg s
...
set_option quotPrecheck false
local notation3 "μ⁺" => SelbergSieve.P520.selbergMuPlus s
...
theorem weight_one_of_selberg : γ 1 = 1 := by
  ...
  rw [inv_mul_cancel₀]
  convert SelbergSieve.P520.selbergBoundingSum_ne_zero s
  rfl

theorem selbergμPlus_eq_zero (d : ℕ) (hd : ¬d ≤ y) : μ⁺ d = 0 := by
  apply lambdaSquared_eq_zero_of_support _ y _ d hd
  apply SelbergSieve.P520.selbergWeights_eq_zero s

def selbergUbSieve : UpperBoundSieve :=
  ⟨μ⁺, upperMoebius_of_lambda_sq γ (SelbergSieve.P520.weight_one_of_selberg s)⟩
...
theorem selberg_bound_simple_mainSum :
  ...
  · apply sum_congr rfl; intro l hl
    rw [SelbergSieve.P520.selbergWeights_diagonalisation s l hl, ite_pow, zero_pow two_ne_zero, mul_ite_zero]
...
theorem selbergBoundingSum_ge {d : ℕ} (hdP : d ∣ P) :
  ...
  _ = (∑ k ∈ divisors P,
    ...
    · intro m hm
      ...
      apply if_ctx_congr _ _ fun _ => rfl
      · exact_mod_cast SelbergSieve.P520._helper s hkd hk hm
      · intro h
  ...
  _ = _ := by
    ...
    · rw [mul_inv_cancel₀, ←Int.cast_pow, moebius_sq_eq_one_of_squarefree]
      ...
      · exact _root_.ne_of_gt <| SelbergSieve.P520.selbergBoundingSum_pos s
...
theorem selberg_bound_weights (d : ℕ) : |γ d| ≤ 1 := by
  by_cases hdP : d ∣ P
  swap
  · rw [SelbergSieve.P520.selbergWeights_eq_zero_of_not_dvd s hdP]; simp only [zero_le_one, abs_zero]
  have : 1*S ≥ γ d * ↑(μ d) * S := by
    rw[one_mul]
    exact SelbergSieve.P520.selbergBoundingSum_ge s hdP
  replace this : γ d * μ d ≤ 1 := by
    apply le_of_mul_le_mul_of_pos_right this (SelbergSieve.P520.selbergBoundingSum_pos s)
  convert this using 1
  rw [← abs_of_nonneg <| SelbergSieve.P520.selbergWeights_mul_mu_nonneg s d hdP,
...
theorem selberg_bound_muPlus (n : ℕ) (hn : n ∈ divisors P) :
  ...
  · gcongr with d1 _ d2
    ...
    · rw [ite_eq_left h, ite_eq_left h]
      apply mul_le_one₀ (SelbergSieve.P520.selberg_bound_weights s d1) (abs_nonneg <| γ d2)
        (SelbergSieve.P520.selberg_bound_weights s d2)
...
theorem selberg_bound_simple_errSum :
  ...
  · apply mul_le_mul _ le_rfl (abs_nonneg <| R d) (pow_nonneg _ <| ω d)
    · apply SelbergSieve.P520.selberg_bound_muPlus s d hd
    · norm_num
  · rw [SelbergSieve.P520.selbergμPlus_eq_zero s d h, abs_zero, zero_mul]

theorem selberg_bound_simple :
    siftedSum (s := s.toBoundingSieve) ≤
      X / S +
        ∑ d ∈ divisors P, if (d : ℝ) ≤ y then (3:ℝ) ^ ω d * |R d| else 0 := by
  let μPlus := SelbergSieve.P520.selbergUbSieve s
  calc
    siftedSum ≤ X * mainSum μPlus + errSum μPlus :=
      siftedSum_le_mainSum_errSum_of_UpperBoundSieve _ μPlus
    _ ≤ _ := ?_
  gcongr
  · erw [SelbergSieve.P520.selberg_bound_simple_mainSum s, div_eq_mul_inv]
  · apply SelbergSieve.P520.selberg_bound_simple_errSum s
