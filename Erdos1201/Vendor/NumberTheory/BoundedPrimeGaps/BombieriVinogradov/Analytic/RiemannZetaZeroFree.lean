...
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.RiemannZetaSelectedSubdivisor
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.ThreeFourOne
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.ZeroFreeRegionArithmetic
...
private lemma re_inv_same_height__RiemannZetaZeroFree (sigma : ℝ) (rho : ℂ) :
...
theorem exists_nat_riemannZeta_zero_re_lt :
  ...
  · have hdist : d ≤ dist rho (1 : ℂ) := by
    ...
    have hgap : d / 2 < 1 - rho.re := by
      ...
      have hdistFormula : dist rho 1 ^ 2 =
          (1 - rho.re) ^ 2 + rho.im ^ 2 := by
        rw [Complex.dist_eq_re_im]
        simp only [Complex.one_re, Complex.one_im, sub_zero]
        rw [Real.sq_sqrt]
        · ring
        · positivity
      rw [hdistFormula] at hdistSq
      nlinarith
      ...
      calc
        _ = ((1 : ℕ) : ℝ) *
          ...
          exact Finsupp.sum_single_index (a := rho) (b := 1)
            ...
            (by norm_num)
        _ = (sigma - rho.re)⁻¹ := by
          rw [Nat.cast_one, one_mul, re_inv_same_height__RiemannZetaZeroFree]
