...
private theorem meromorphicOrderAt_selectedRawFactor
  ...
  rw [selectedRawFactor,
    meromorphicOrderAt_mul (hzero z |>.inv) hfz.meromorphicAt,
    meromorphicOrderAt_inv,
    selectedZeroProduct,
    Function.FactorizedRational.meromorphicOrderAt_eq
      (selectedDivisor f c R)
        ((selectedDivisor f c R).finiteSupport
          (isCompact_closedBall c (2 * R))),
  ...
  have hP_analytic : AnalyticAt ℂ (selectedZeroProduct f c R) z := by
    ...
    exact Function.FactorizedRational.analyticAt
      (MeromorphicOn.AnalyticOnNhd.divisor_nonneg
        (hf.mono (closedBall_subset_closedBall (by linarith))) z)
...
private theorem norm_selectedRegularFactor_le_on_outer_sphere
  ...
  have hPc : selectedZeroProduct f c R c ≠ 0 := by
    simpa [selectedZeroProduct] using
      (Function.FactorizedRational.ne_zero (d := selectedDivisor f c R) hDc)
  have hPw : selectedZeroProduct f c R w ≠ 0 := by
    simpa [selectedZeroProduct] using
      (Function.FactorizedRational.ne_zero (d := selectedDivisor f c R) hDw)
...
private theorem logDeriv_factorizedRational_eq_finsum
  ...
  -- v4.34 port: keep the ∏-of-functions shape so `logDeriv_prod` matches
  -- syntactically (respectTransparency); each factor is a `rfl` reshaping.
  rw [show (∏ rho ∈ hD.toFinset, (fun z : ℂ => z - rho) ^ D rho)
      = ∏ rho ∈ hD.toFinset, (fun z : ℂ => (z - rho) ^ D rho) from
    Finset.prod_congr rfl fun rho _ => rfl, logDeriv_prod]
...
private theorem logDeriv_selectedRegularFactor_eq_sub_finsum
  ...
  have hPs : selectedZeroProduct f c R s ≠ 0 := by
    simpa [selectedZeroProduct] using
      (Function.FactorizedRational.ne_zero (d := selectedDivisor f c R) hDs)
  have hPa : AnalyticAt ℂ (selectedZeroProduct f c R) s := by
    unfold selectedZeroProduct
    exact Function.FactorizedRational.analyticAt
      (MeromorphicOn.AnalyticOnNhd.divisor_nonneg
        (hf.mono (closedBall_subset_closedBall (by linarith))) s)
  ...
  rw [hlogeq]
  -- v4.34 port: produce the Pi-division form so `logDeriv_div` matches.
  have hrawfun : selectedRawFactor f c R =
      f / selectedZeroProduct f c R := by
    funext z
  ...
  rw [selectedZeroProduct, logDeriv_factorizedRational_eq_finsum
    ((selectedDivisor f c R).finiteSupport
      (isCompact_closedBall c (2 * R))) hDs]
