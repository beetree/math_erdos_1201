...
theorem exists_norm_gammaFactor_ratio_fixedStrip_le :
  ...
  · have heven_inv : (chi⁻¹).Even := by
      simp only [DirichletCharacter.Even] at heven ⊢
      rw [MulChar.inv_apply_eq_inv', heven, inv_one]
    rw [heven_inv.gammaFactor_def, heven.gammaFactor_def]
    simpa using hbound 0 (by norm_num) (by norm_num)
  · have hodd_inv : (chi⁻¹).Odd := by
      simp only [DirichletCharacter.Odd] at hodd ⊢
      rw [MulChar.inv_apply_eq_inv', hodd]
      norm_num
    rw [hodd_inv.gammaFactor_def, hodd.gammaFactor_def]
