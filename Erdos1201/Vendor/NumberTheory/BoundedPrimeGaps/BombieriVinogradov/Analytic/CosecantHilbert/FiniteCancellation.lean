...
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.CosecantHilbert.FiniteExpansion
...
def montgomeryVaughanA (x : ι → ℝ) (u : ι → ℂ) : ℂ :=
...
def montgomeryVaughanB (x : ι → ℝ) (u : ι → ℂ) : ℂ :=
  ∑ r, ∑ s ∈ Finset.univ.erase r,
    ∑ t ∈ (Finset.univ.erase r).erase s,
      star (u s) * u t *
        (((Real.sin (Real.pi * (x s - x t)))⁻¹ : ℝ) : ℂ) *
...
lemma cscCotangentKernel_isHermitian (x : ι → ℝ) :
  ...
  · rw [cscCotangentKernel, cscCotangentKernel, ite_eq_right hsr,
      ite_eq_right (Ne.symm hsr)]
...
lemma montgomeryVaughanS5_eq_star_dotProduct_mulVec
  ...
  rw [cscCotangentKernel, ite_eq_right (Ne.symm hrs)]
  rw [Pi.star_apply]
  ring
