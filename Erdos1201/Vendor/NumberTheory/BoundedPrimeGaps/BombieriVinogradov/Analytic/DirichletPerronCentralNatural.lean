...
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.DirichletPerronCentral
...
private lemma abs_half_add_sincIntegral_sub_step_le
  ...
  · rw [min_eq_right ((div_le_one hAabs).2 hlarge)]
    ...
    by_cases hApos : 0 < A
    · rw [ite_eq_left hApos, Real.sign_of_pos hApos] at *
    ...
    · have hAneg : A < 0 := lt_of_le_of_ne (le_of_not_gt hApos) hA
      rw [ite_eq_right hApos, Real.sign_of_neg hAneg] at *
      have htailS : |S + Real.pi / 2| ≤ |A| ^ (-1 : ℤ) := by
        simpa [S] using htail
      calc
        |1 / 2 + S / Real.pi - 0| =
