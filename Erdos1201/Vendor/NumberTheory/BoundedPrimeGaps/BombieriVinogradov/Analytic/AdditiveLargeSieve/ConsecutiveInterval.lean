...
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.CosecantHilbert
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.AdditiveLargeSieve.FiniteDuality
public import Erdos1201.Vendor.NumberTheory.BoundedPrimeGaps.BombieriVinogradov.Analytic.AdditiveLargeSieve.ConsecutiveIntervalKernel
...
noncomputable def phaseInterval (x : ℝ) : ℂ :=
  Complex.exp (((2 * Real.pi * x : ℝ) : ℂ) * Complex.I)

def lift (x : UnitAddCircle) : ℝ :=
  AddCircle.equivIco 1 (0 : ℝ) x

private lemma coe_lift (x : UnitAddCircle) : (lift x : UnitAddCircle) = x := by
  exact AddCircle.coe_equivIco

private lemma phase_norm (x : ℝ) : ‖phaseInterval x‖ = 1 := by
  rw [phaseInterval]
  exact Complex.norm_exp_ofReal_mul_I _

private lemma char_coe_phase (x : ℝ) (n : ℕ) :
    unitAddCircleAddChar (n • (x : UnitAddCircle)) = phaseInterval ((n : ℝ) * x) := by
  unfold unitAddCircleAddChar phaseInterval
...
private lemma char_lift_phase (x : UnitAddCircle) (n : ℕ) :
    unitAddCircleAddChar (n • x) = phaseInterval ((n : ℝ) * lift x) := by
  calc
    unitAddCircleAddChar (n • x) =
        unitAddCircleAddChar (n • (lift x : UnitAddCircle)) := by
      rw [coe_lift]
    _ = phaseInterval ((n : ℝ) * lift x) := char_coe_phase (lift x) n

private lemma phase_sub_factor (a x y : ℝ) :
    phaseInterval (a * (x - y)) = phaseInterval (a * x) * star (phaseInterval (a * y)) := by
  unfold phaseInterval
...
private lemma sum_phase_norm_sq_expand
    ...
    (((∑ n ∈ S, ‖∑ r, phaseInterval ((n : ℝ) * y r) * v r‖ ^ 2 : ℝ) : ℂ)) =
      ∑ r, ∑ s,
        v r * star (v s) *
          ∑ n ∈ S, phaseInterval ((n : ℝ) * (y r - y s)) := by
  calc
    (((∑ n ∈ S, ‖∑ r, phaseInterval ((n : ℝ) * y r) * v r‖ ^ 2 : ℝ) : ℂ)) =
        ∑ n ∈ S,
          (∑ r, phaseInterval ((n : ℝ) * y r) * v r) *
            star (∑ s, phaseInterval ((n : ℝ) * y s) * v s) := by
      ...
      ring
    _ = ∑ n ∈ S, ∑ r, ∑ s,
          (phaseInterval ((n : ℝ) * y r) * v r) *
            star (phaseInterval ((n : ℝ) * y s) * v s) := by
    ...
    _ = ∑ r, ∑ s,
      ...
      calc
        phaseInterval ((n : ℝ) * y r) * v r *
            ((starRingEnd ℂ) (phaseInterval ((n : ℝ) * y s)) *
              (starRingEnd ℂ) (v s)) =
            (phaseInterval ((n : ℝ) * y r) *
              (starRingEnd ℂ) (phaseInterval ((n : ℝ) * y s))) *
              (v r * (starRingEnd ℂ) (v s)) := by ring
        _ = phaseInterval ((n : ℝ) * (y r - y s)) *
              (v r * (starRingEnd ℂ) (v s)) := by rw [← hphase]
        _ = v r * (starRingEnd ℂ) (v s) *
              phaseInterval ((n : ℝ) * (y r - y s)) := by ring

private lemma sum_phase_diag (S : Finset ℕ) (y : ℝ) :
    ∑ n ∈ S, phaseInterval ((n : ℝ) * (y - y)) = (S.card : ℂ) := by
  simp [phaseInterval]
...
private lemma sum_phase_norm_sq_eq_diag_add_offDiagonal
  ...
  calc
    (∑ r, ∑ s,
        v r * star (v s) *
          ∑ n ∈ S, phaseInterval ((n : ℝ) * (y r - y s))) =
        ∑ r, (v r * star (v r) *
            ∑ n ∈ S, phaseInterval ((n : ℝ) * (y r - y r)) +
...
private lemma sum_phase_Ioc_eq_cosecant
    ...
    (∑ n ∈ Finset.Ioc m0 (m0 + N), phaseInterval ((n : ℝ) * t)) =
      Complex.I / 2 *
        (phaseInterval (((m0 : ℝ) + 1 / 2) * t) -
          phaseInterval ((((m0 + N : ℕ) : ℝ) + 1 / 2) * t)) *
...
private lemma offDiagonal_eq_endpoint_forms
    ...
    (∑ r, ∑ s ∈ Finset.univ.erase r,
      v r * star (v s) *
        ∑ n ∈ Finset.Ioc m0 (m0 + N),
          phaseInterval ((n : ℝ) * (y r - y s))) =
      Complex.I / 2 *
        (cosecantBilinearForm y
            (fun r => v r * phaseInterval (((m0 : ℝ) + 1 / 2) * y r)) -
          cosecantBilinearForm y
            (fun r => v r *
              phaseInterval ((((m0 + N : ℕ) : ℝ) + 1 / 2) * y r))) := by
...
private lemma phase_twist_energy
    {ι : Type*} [Fintype ι]
    (a : ℝ) (y : ι → ℝ) (v : ι → ℂ) :
    (∑ r, ‖v r * phaseInterval (a * y r)‖ ^ 2) = ∑ r, ‖v r‖ ^ 2 := by
  apply Finset.sum_congr rfl
  intro r _
...
private lemma norm_offDiagonal_le
  ...
  let u₀ : ι → ℂ :=
    fun r => v r * phaseInterval (((m0 : ℝ) + 1 / 2) * y r)
  let u₁ : ι → ℂ :=
    fun r => v r * phaseInterval ((((m0 + N : ℕ) : ℝ) + 1 / 2) * y r)
...
private lemma sum_phase_norm_sq_Ioc_le
  ...
  have hre' :
      (∑ n ∈ S, ‖∑ r, phaseInterval ((n : ℝ) * y r) * v r‖ ^ 2) =
        (S.card : ℝ) * E + O.re := by
