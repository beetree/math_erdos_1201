...
public import Erdos1201.Vendor.NumberTheory.PNT.Auxiliary
public import Erdos1201.Vendor.Analysis.FourierW21
public import Erdos1201.Vendor.NumberTheory.PNT.ResidueCalcOnRectangles
public import Erdos1201.Vendor.NumberTheory.PNT.EulerMaclaurin
...
theorem ResidueOfTendsTo {f : ℂ → ℂ} {p : ℂ} {U : Set ℂ}
  ...
  have h_prod_holo : DifferentiableOn ℂ (fun z ↦ (z - p) * f z) (W \ {p}) := by
    have h_id : DifferentiableOn ℂ (fun z : ℂ ↦ z - p) (W \ {p}) :=
      Differentiable.differentiableOn (Differentiable.sub_const differentiable_fun_id p)
    have hfW : DifferentiableOn ℂ f (W \ {p}) := by
      apply hf.mono
...
theorem differentiableAt_deriv_riemannZeta {s : ℂ} (s_ne_one : s ≠ 1) :
  ...
  exact (analyticOn_riemannZeta s s_ne_one).deriv.differentiableAt



theorem riemannZetaResidue :
    ∃ U ∈ 𝓝 1, BddAbove (norm ∘ (ζ - (fun s ↦ (s - 1)⁻¹)) '' (U \ {1})) := by
  have zeta_holc : DifferentiableOn ℂ ζ (univ \ {1}) := by
    intro y hy
...
theorem analytic_deriv_bounded_near_point
    (f : ℂ → ℂ) {U : Set ℂ} {p : ℂ} (hU : IsOpen U) (hp : p ∈ U) (hf : DifferentiableOn ℂ f U) :
...
theorem logDerivResidue' {f : ℂ → ℂ} {p : ℂ} {U : Set ℂ}
    ...
    (holc : DifferentiableOn ℂ f (U \ {p}))
    (U_in_nhds : U ∈ 𝓝 p) {A : ℂ} (A_ne_zero : A ≠ 0)
    (f_near_p : BddAbove (norm ∘ (f - fun s ↦ A * (s - p)⁻¹) '' (U \ {p}))) :
    (deriv f * f⁻¹ + (fun s ↦ (s - p)⁻¹)) =O[𝓝[≠] p] (1 : ℂ → ℂ) := by


  have simpleHolo : DifferentiableOn ℂ (fun s ↦ A / (s - p)) (U \ {p}) := by
  ...
  have f_minus_pole_is_holomorphic : DifferentiableOn ℂ (f - (fun s ↦ A * (s - p)⁻¹)) (U \ {p}) := by
  ...
  have linear_is_holomorphic : DifferentiableOn ℂ (fun (s : ℂ ) ↦ (s - p)) U := by
    exact DifferentiableOn.sub_const differentiableOn_id p

  have h_is_holomorphic : DifferentiableOn ℂ h U := by
...
theorem ResidueMult {f g : ℂ → ℂ} {p : ℂ} {U : Set ℂ}
    (g_holc : DifferentiableOn ℂ g U) (U_in_nhds : U ∈ 𝓝 p) {A : ℂ}
...
theorem riemannZetaLogDerivResidue :
  ...
  have ζ_holc: DifferentiableOn ℂ ζ (W \ {1}) := by
    intro y hy
...
lemma riemannZeta0_apply (N : ℕ) (s : ℂ) : ζ₀ N s =
  ...
  simp_rw [riemannZeta0, div_eq_mul_inv, ← Complex.cpow_neg]; ring
...
lemma ZetaSum_aux1₁ {a b : ℕ} {s : ℂ} (s_ne_one : s ≠ 1) (ha : a ∈ Ioo 0 b) :
  ...
  · refine intervalIntegral.integral_congr fun x hx ↦ ?_
    simpa [one_div] using (Complex.cpow_neg (x : ℂ) s).symm
  · norm_cast; ring_nf
...
lemma ZetaSum_aux1_2 {a b : ℝ} {c : ℝ} (apos : 0 < a) (a_lt_b : a < b)
  ...
  change 1 / x ^ (c + 1) = x ^ (-c - 1)
  rw [div_eq_mul_inv, ← Real.rpow_neg this]
  ring_nf
...
lemma ZetaSum_aux1_5a {a b : ℝ} (apos : 0 < a) {s : ℂ} (x : ℝ)
  ...
  · apply Real.rpow_nonneg <| le_of_lt (lt_of_lt_of_le apos h.1)
...
lemma ZetaSum_aux1_5b {a b : ℝ} (apos : 0 < a) (a_lt_b : a < b) {s : ℂ} (σpos : 0 < s.re) :
  ...
  · exact continuousOn_id.rpow_const fun x hx ↦ Or.inl (ne_of_gt <| lt_of_lt_of_le apos hx.1)
  · exact fun x hx h ↦ by rw [Real.rpow_eq_zero] at h <;> linarith [lt_of_lt_of_le apos hx.1]
...
lemma ZetaSum_aux3 {N : ℕ} {s : ℂ} (s_re_gt : 1 < s.re) :
  ...
  simp_rw [(Finset.Ico_add_one_add_one_eq_Ioc _ _).symm]
...
lemma Zeta0EqZeta {N : ℕ} (N_pos : 0 < N) {s : ℂ} (reS_pos : 0 < s.re) (s_ne_one : s ≠ 1) :
  ...
  have f_an : AnalyticOnNhd ℂ f U := by
    apply (differentiableOn_riemannZeta.analyticOnNhd isOpen_compl_singleton).mono
    intro a ha
    exact ha.1
...
lemma DerivZeta0EqDerivZeta {N : ℕ} (N_pos : 0 < N) {s : ℂ} (reS_pos : 0 < s.re)
  ...
  intro x hx
  have hζ := differentiableOn_riemannZeta.mono (by intro x hx; exact hx.1) |>.hasDerivAt (s := U) <|
...
lemma ZetaUpperBnd' {A σ t : ℝ} (hA : A ∈ Ioc 0 (1 / 2)) (t_gt : 3 < |t|)
  ...
  · simp only [add_le_add_iff_right]
    simp_rw [one_div, ← Complex.cpow_neg]
...
lemma DerivUpperBnd_aux1 {A C σ t : ℝ} (hA : A ∈ Ioc 0 (1 / 2))
  ...
  rw [mul_assoc]
  apply le_trans this <| (mul_le_mul_iff_right₀ A.exp_pos).mpr ?_
  rw [pow_two, ← mul_assoc, Finset.range_eq_Ico, Finset.Ico_add_one_right_eq_Icc, harmonic_eq_sum_Icc0]
...
theorem DerivUpperBnd_aux5 {A σ t : ℝ} (t_gt : 3 < |t|) (hσ : σ ∈ Icc (1 - A / |t|.log) 2) :
  ...
  · have hσ : σ ∈ Ioc 0 2 := ⟨(by linarith), hσ.2⟩
    ...
    simp_rw [← Complex.cpow_neg] at this
    rw [show -(↑σ + ↑t * I) - 1 = -(↑σ + ↑t * I + 1) by ring]
...
lemma DerivUpperBnd_aux7_tendsto {σ : ℝ} (σpos : 0 < σ) :
  ...
  have h3 : Tendsto (fun t : ℝ ↦ t ^ (-σ) * Real.log t) atTop (nhds 0) := by
    have := (isLittleO_log_rpow_rpow_atTop 1 σpos).tendsto_div_nhds_zero
    simp only [Real.rpow_one] at this
    apply Tendsto.congr' _ this
    filter_upwards [eventually_ge_atTop 0] with x hx
    rw [mul_comm]
    rw [div_eq_mul_inv, ← Real.rpow_neg hx σ]
...
lemma deriv_fun_re {t : ℝ} {f : ℂ → ℂ} (diff : ∀ (σ : ℝ), DifferentiableAt ℂ f (↑σ + ↑t * I)) :
  ...
  · simp only [deriv_add_const', _root_.deriv_ofReal, mul_one] at this
    exact this
  · apply DifferentiableAt.add_const _ <| differentiable_ofReal.differentiableAt
...
lemma Zeta_eq_int_derivZeta {σ₁ σ₂ t : ℝ} (t_ne_zero : t ≠ 0) :
  ...
  · intro s _
    ...
    · exact DifferentiableAt.add_const (c := t * I) <| differentiable_ofReal.differentiableAt
...
lemma ZetaNoZerosInBox (T : ℝ) :
  ...
  · have zetaIsZero : ζ (1 + Complex.I * t₀) = 0 := by
      have cont : ContinuousOn ζ (univ \ {1}) := by
        simpa [Set.compl_eq_univ_sdiff] using differentiableOn_riemannZeta.continuousOn
...
lemma LogDerivZetaHoloOn {S : Set ℂ} (s_ne_one : 1 ∉ S)
    (nonzero : ∀ s ∈ S, ζ s ≠ 0) :
    DifferentiableOn ℂ (fun s ↦ ζ' s / ζ s) S := by
...
theorem LogDerivZetaHolcSmallT :
    ∃ (σ₂ : ℝ) (_ : σ₂ < 1), DifferentiableOn ℂ (fun (s : ℂ) ↦ ζ' s / (ζ s))
      (( [[ σ₂, 2 ]] ×ℂ [[ -3, 3 ]]) \ {1}) := by
...
theorem LogDerivZetaHolcLargeT :
    ...
    DifferentiableOn ℂ (fun (s : ℂ) ↦ ζ' s / (ζ s))
