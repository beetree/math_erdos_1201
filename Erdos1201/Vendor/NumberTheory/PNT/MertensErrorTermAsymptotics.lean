...
/-!
# Mertens' theorems with explicit error terms, via the log-zeta / Gamma-derivative route

This file derives the classical asymptotics of `∑ Λ(n)/n`, `∑ log p / p`,
`∑ 1/p`, and `∏ (1 - 1/p)` over `n ≤ x` (resp. primes `p ≤ x`), each with an
explicit, bounded (indeed decaying) error term, by relating `log ζ(s)` near
`s = 1` to `Γ'(1)` and the Euler–Mascheroni constant `γ` through an
integral representation of `log ζ` and its behaviour as `s → 1⁺`.
-/
...
namespace MertensErrorTermAsymptotics

section GammaLogIntegralAux
...
/-- Core identity, stated with explicit qualifiers (outside the main namespace,
where `Finset` is open and would clash with `Set.Ioi`). -/
theorem mul_integ_log_log_eq_aux (s : ℝ) (hs : 1 < s) :
...
lemma tendsto_antideriv_log_div_sq :
...
theorem integrable_E₁Λ_div_mul_log_sq {x : ℝ} (hx : 2 ≤ x) :
...
-- Helpers for `log_zeta_eq_integ`: Abel summation / sum-integral interchange.
...
this by domination. -/
lemma summable_log_rpow_div_rpow (a : ℝ) {s : ℝ} (hs : 1 < s) :
...
lemma summable_vonMangoldt_div_rpow (s : ℝ) (hs : 1 < s) :
...
lemma summable_c_term (s : ℝ) (hs : 1 < s) :
...
theorem mul_integ_log_log_eq (s : ℝ) (hs : 1 < s) :
    (s - 1) * ∫ x in .Ioi 1, log (log x) * x^(-s) = - log (s - 1) + deriv Gamma 1 :=
  mul_integ_log_log_eq_aux s hs


theorem mul_integ_gamma_eq (s) (hs : 1 < s) : (s - 1) * ∫ x in .Ioi 1, γ * x^(-s) = γ := by
  rw [MeasureTheory.integral_const_mul γ (· ^ (-s)), @integral_Ioi_rpow_of_lt (-s), one_rpow] <;>
    grind

-- Integrability helpers for the integral splitting in `log_zeta_eq`.
...
theorem integrableOn_Ioi_mul_rpow_neg_of_abs_le
...
theorem integrableOn_γ_mul_rpow (s : ℝ) (hs : 1 < s) :
...
theorem integrableOn_E₂Λ_mul_rpow (s : ℝ) (hs : 1 < s) :
...
theorem log_zeta_eq (s : ℝ) (hs : 1 < s) :
  ...
  have key : (∫ x in Set.Ioi 1, (log (log x) + γ + E₂Λ x) * x ^ (-s))
    ...
    intro x _
    ring
  -- Apply the auxiliary integral identities, then finish algebraically.
  rw [key, mul_add, mul_add, mul_integ_log_log_eq s hs, mul_integ_gamma_eq s hs]

lemma zeta_pole_mul_re_tendsto_one :
...
-- Helpers for `deriv_gamma_add_γ_eq_zero`: take `s → 1⁺` in `log_zeta_eq`.
section
open MeasureTheory Set

/-- `E₂Λ` is measurable: its Mangoldt-sum part factors through `⌊·⌋₊` and the rest is
continuous/measurable. -/
lemma measurable_E₂Λ : Measurable E₂Λ := by fun_prop

/-- On `(1,2)` the Mangoldt sum is empty (`⌊x⌋₊ = 1`), so `E₂Λ x = - log (log x) - γ`. -/
lemma E₂Λ_eq_on_Ioo {x : ℝ} (hx : x ∈ Set.Ioo (1 : ℝ) 2) :
...
lemma abs_E₂Λ_le_on_Ioo {x : ℝ} (hx : x ∈ Set.Ioo (1 : ℝ) 2) :
...
lemma abs_E₂Λ_le_const {x : ℝ} (hx : 2 ≤ x) :
    |E₂Λ x| ≤ (log 4 + 6) / log 2 :=
...
lemma integrableOn_log_sub_one_bound :
...
lemma integrableOn_E₂Λ_Ioo {X : ℝ} (_hX : 2 ≤ X) :
...
lemma sub_one_mul_integral_E₂Λ_tendsto :
...
end MertensErrorTermAsymptotics
