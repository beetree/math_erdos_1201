...
public import Erdos1201.Vendor.NumberTheory.PNT.MellinCalculus
public import Erdos1201.Vendor.NumberTheory.PNT.ZetaBounds
public import Erdos1201.Vendor.NumberTheory.PNT.ZetaConj
public import Erdos1201.Vendor.Analysis.CarlesonOperator.Classical.Helper
public import Erdos1201.Vendor.Analysis.SmoothExistence
...
theorem SmoothedChebyshevPull1 {SmoothingF : ℝ → ℝ} {ε : ℝ} (ε_pos : 0 < ε)
    ...
    (holoOn : DifferentiableOn ℂ (ζ' / ζ) ((Icc σ₁ 2) ×ℂ (Icc (-T) T) \ {1}))
    (suppSmoothingF : Function.support SmoothingF ⊆ Icc (1 / 2) 2)
    (SmoothingFnonneg : ∀ x > 0, 0 ≤ SmoothingF x)
    (mass_one : ∫ x in Ioi 0, SmoothingF x / x = 1)
    (ContDiffSmoothingF : ContDiff ℝ 1 SmoothingF) :
...
theorem SmoothedChebyshevPull2 {SmoothingF : ℝ → ℝ} {ε : ℝ} (ε_pos : 0 < ε) (ε_lt_one : ε < 1)
    ...
    (σ₂_lt_σ₁ : σ₂ < σ₁)
    (holoOn : DifferentiableOn ℂ (ζ' / ζ) ((Icc σ₁ 2) ×ℂ (Icc (-T) T) \ {1}))
    (holoOn2 : DifferentiableOn ℂ (SmoothedChebyshevIntegrand SmoothingF ε X)
      (Icc σ₂ 2 ×ℂ Icc (-3) 3 \ {1}))
    (suppSmoothingF : Function.support SmoothingF ⊆ Icc (1 / 2) 2)
    (SmoothingFnonneg : ∀ x > 0, 0 ≤ SmoothingF x)
    (mass_one : ∫ x in Ioi 0, SmoothingF x / x = 1)
  ...
  have sub : z.Rectangle w ⊆ Icc σ₂ 2 ×ℂ Icc (-3) 3 \ {1} := by
    ...
    · simp only [mem_singleton_iff]
      ...
      have h_x_ne_one : x ≠ 1 := by
        ...
        linarith
      exact h_x_ne_one
  have zero_over_box : RectangleIntegral (SmoothedChebyshevIntegrand SmoothingF ε X) z w = 0 := by
  ...
  suffices ∀ σ ∈ Ioc σ₁ (1 + (Real.log X)⁻¹),
    ...
    calc
      ‖∫ (σ : ℝ) in σ₁..1 + (Real.log X)⁻¹,
        ...
        simpa only [uIoc_of_le interval_length_nonneg] using this
      _ ≤ C' * X / (ε * T) * 2 := by
...
lemma I7I3 {SmoothingF : ℝ → ℝ} {ε X T σ₁ : ℝ} (Xpos : 0 < X) :
  ...
  congr
  · ring
  · rw [← intervalIntegral.intervalIntegral_conj, ← intervalIntegral.integral_comp_neg]
    apply intervalIntegral.integral_congr
    intro t ht
    simp only
    rw [← smoothedChebyshevIntegrand_conj Xpos]
...
set_option maxHeartbeats 400000 in
-- Slow
/-- *** Prime Number Theorem (Medium Strength) *** The `Chebyshev.psi` function is asymptotic to `x`. -/
...
theorem MediumPNT : ∃ c > 0,
  ...
  have holo2 : DifferentiableOn ℂ (fun s ↦ ζ' s / ζ s) (uIcc σ₂ 2 ×ℂ uIcc (-3) 3 \ {1}) := by
    apply holo2'.mono
    intro s hs
  ...
  have holo2a : DifferentiableOn ℂ (SmoothedChebyshevIntegrand ν ε X)
...
/-- The prime-power contribution is `O(√x log x)`. -/
theorem theta_sub_psi_isBigO_sqrt_mul_log :
