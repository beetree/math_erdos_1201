...
lemma PartialIntegration_of_support_in_Icc {a b : ℝ} (f g : ℝ → ℂ) (ha : 0 < a)
  ...
  simpa using MeasureTheory.integral_Ioi_mul_deriv_eq_deriv_mul
    (a := 0) (a' := (0 : ℂ)) (b' := (0 : ℂ))
    (u := f) (v := g) (u' := deriv f) (v' := deriv g)
...
lemma MellinConvolutionTransform (f g : ℝ → ℂ) (s : ℂ)
  ...
  calc
    ...
    _ = ∫ (y : ℝ) in Ioi 0, ∫ (x : ℝ) in Ioi 0, f₁ (x, y) :=
        by
          have hprod : Integrable (Function.uncurry fun (x : ℝ) (y : ℝ) ↦ f₁ (x, y))
...
lemma MellinOfPsi {ν : ℝ → ℝ} (diffν : ContDiff ℝ 1 ν)
  ...
  have mainBnd : ∀ (σ₁ : ℝ), 0 < σ₁ → ∀ (s : ℂ), σ₁ ≤ s.re → s.re ≤ 2 →
    ...
    · have := intervalIntegral.norm_integral_le_of_norm_le_const (C := f a * 2 ^ σ₂)
        (f := fun x ↦ f x * ‖(x : ℂ) ^ s‖) (a := (1 / 2 : ℝ)) ( b := 2) ?_
...
lemma MellinOfSmooth1a {ν : ℝ → ℝ} (diffν : ContDiff ℝ 1 ν)
  ...
  · have hMellinOne : 𝓜 (fun x ↦ if 0 < x ∧ x ≤ 1 then 1 else 0) s = 1 / s := by
      convert (hasMellin_one_Ioc hs).right
      congr
    rw [hMellinOne, MellinOfDeltaSpike ν εpos s]
    simp
