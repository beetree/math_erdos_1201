...
public import Erdos1201.Vendor.NumberTheory.PNT.Rectangle
...
theorem existsDifferentiableOn_of_bddAbove [CompleteSpace E]
    ...
    ∃ (g : ℂ → E),
      DifferentiableOn ℂ g s ∧ Set.EqOn f g (s \ {c}) :=
...
theorem HolomorphicOn.rectangleBorderIntegrable'
    (hf : DifferentiableOn ℂ f (Rectangle z w \ {p})) (hp : Rectangle z w ∈ nhds p) :
    RectangleBorderIntegrable f z w :=
  hf.continuousOn.rectangleBorderNoPIntegrable (not_mem_rectangleBorder_of_rectangle_mem_nhds hp)

theorem HolomorphicOn.rectangleBorderIntegrable (hf : DifferentiableOn ℂ f (Rectangle z w)) :
...
lemma RectanglePullToNhdOfPole' [CompleteSpace E] {z₀ z₁ z₂ z₃ p : ℂ}
    ...
    (fHolo : DifferentiableOn ℂ f (Rectangle z₀ z₃ \ {p})) :
  ...
  have h₂ : RectangleIntegral f z₀ (↑z₃.re + ↑z₁.im * I) = 0 := by
    unfold RectangleIntegral VIntegral
    exact Complex.integral_boundary_rect_eq_zero_of_differentiableOn _ _ _
      (fHolo.mono (by simpa [Rectangle] using hbot''))
  have h₃ := RectangleIntegralVSplit' htop' htop
  have h₄ : RectangleIntegral f (↑z₀.re + ↑z₂.im * I) z₃ = 0 := by
    unfold RectangleIntegral VIntegral
    exact Complex.integral_boundary_rect_eq_zero_of_differentiableOn _ _ _
      (fHolo.mono (by simpa [Rectangle] using htop''))
  have h₅ := RectangleIntegralHSplit' hleft' hleft
  have h₆ : RectangleIntegral f (↑z₀.re + ↑z₁.im * I) (↑z₁.re + ↑z₂.im * I) = 0 := by
    unfold RectangleIntegral VIntegral
    exact Complex.integral_boundary_rect_eq_zero_of_differentiableOn _ _ _
      (fHolo.mono (by simpa [Rectangle] using hleft''))
  have h₇ := RectangleIntegralHSplit' hright' hright
  have h₈ : RectangleIntegral f (↑z₂.re + ↑z₁.im * I) (↑z₃.re + ↑z₂.im * I) = 0 := by
    unfold RectangleIntegral VIntegral
    exact Complex.integral_boundary_rect_eq_zero_of_differentiableOn _ _ _
      (fHolo.mono (by simpa [Rectangle] using hright''))
...
lemma RectanglePullToNhdOfPole [CompleteSpace E] {z w p : ℂ}
    ...
    (hp : Rectangle z w ∈ 𝓝 p) (fHolo : DifferentiableOn ℂ f (Rectangle z w \ {p})) :
    ∀ᶠ (c : ℝ) in 𝓝[>]0,
...
lemma ResidueTheoremOnRectangleWithSimplePole {f g : ℂ → ℂ} {z w p A : ℂ}
    (zRe_le_wRe : z.re ≤ w.re) (zIm_le_wIm : z.im ≤ w.im)
    (pInRectInterior : Rectangle z w ∈ 𝓝 p) (gHolo : DifferentiableOn ℂ g (Rectangle z w))
  ...
  have t2 : DifferentiableOn ℂ (fun s ↦ A / (s - p)) (Rectangle z w \ {p}) := by
  ...
  have hgZero : RectangleIntegral g z w = 0 := by
    unfold RectangleIntegral VIntegral
    exact Complex.integral_boundary_rect_eq_zero_of_differentiableOn _ _ _ (gHolo.mono (by rfl))
  rw [hgZero, smul_zero, zero_add]
...
theorem ResidueTheoremOnRectangleWithSimplePole' {f : ℂ → ℂ} {z w p A : ℂ}
    (zRe_le_wRe : z.re ≤ w.re) (zIm_le_wIm : z.im ≤ w.im)
    (pInRectInterior : Rectangle z w ∈ 𝓝 p) (fHolo : DifferentiableOn ℂ f (Rectangle z w \ {p}))
    (near_p : (f - (fun s ↦ A / (s - p))) =O[𝓝[≠] p] (1 : ℂ → ℂ)) :
    RectangleIntegral' f z w = A := by
  set g := f - (fun s ↦ A / (s - p))
  have gHolo : DifferentiableOn ℂ g (Rectangle z w \ {p}) := by
...
lemma holoPart_holomorphicOn {f : ℂ → ℂ} {z w : ℂ}
    ...
    DifferentiableOn ℂ (toMeromorphicNFOn (toMeromorphicNFOn f (Rectangle z w) -
...
lemma toMeromorphicNFOn_add_integral {f : ℂ → ℂ} {z w : ℂ}
  ...
  have h_holoPart_border : RectangleBorderIntegrable holoPart z w :=
    HolomorphicOn.rectangleBorderIntegrable
      (holoPart_holomorphicOn f_mero f_simple_poles f_poles_finite)
  have h_fNF_eq :
