...
lemma contDiff_ofReal : ContDiff ℝ ∞ ofReal :=
...
lemma tendsto_scale (f : CS n E) (x : ℝ) : Tendsto (fun R => f.scale R x) atTop (𝓝 (f 0)) := by
  apply (tendsto_funscale f.h1.continuous.continuousAt x).congr'
  filter_upwards [eventually_ne_atTop 0] with R hR ; simp [scale, hR]

lemma bounded : ∃ C, ∀ v, ‖f v‖ ≤ C := by
  obtain ⟨x, hx⟩ :=
    (continuous_norm.comp f.h1.continuous).exists_forall_ge_of_hasCompactSupport f.h2.norm
  exact ⟨_, hx⟩
...
def sub (f g : W1 n E) : W1 n E where
  ...
  integrable k hk := by
    ...
    have e : iteratedDeriv k ((f : ℝ → E) - (g : ℝ → E)) =
        iteratedDeriv k (f : ℝ → E) - iteratedDeriv k (g : ℝ → E) := by
      funext x ; exact iteratedDeriv_sub hf.contDiffAt hg.contDiffAt
    show Integrable (iteratedDeriv k ((f : ℝ → E) - (g : ℝ → E))) volume
    rw [e]
    exact (f.integrable hk).sub (g.integrable hk)
...
theorem W21_approximation (f : W21) (g : trunc) :
  ...
  have ch {R} : Continuous (fun v => (h R v : ℂ)) :=
    continuous_ofReal.comp <| continuous_const.sub (g.scale R).h1.continuous
  have ch' {R} : Continuous (fun v => (h' R v : ℂ)) :=
    continuous_ofReal.comp (h' R).h1.continuous
  have ch'' {R} : Continuous (fun v => (h'' R v : ℂ)) :=
    continuous_ofReal.comp (h'' R).h1.continuous
  ...
  · let F R v := ‖h R v * f v‖
    ...
    have e1 : ∀ᶠ (n : ℝ) in atTop, AEStronglyMeasurable (F n) volume := by
      ...
      exact (ch.mul f.smooth.continuous).norm.aestronglyMeasurable
  ...
  · let F R v := ‖h'' R v * f v + 2 * h' R v * f' v + h R v * f'' v‖
    ...
    have e1 : ∀ᶠ (n : ℝ) in atTop, AEStronglyMeasurable (F n) volume := by
      ...
      exact ((ch''.mul f.smooth.continuous).add
        ((continuous_const.mul ch').mul f.deriv.smooth.continuous)).add
        (ch.mul f.deriv.deriv.smooth.continuous)
    ...
    have e4 : ∀ᵐ (a : ℝ), Tendsto (fun n ↦ F n a) atTop (𝓝 0) := by
      ...
      · rw [Filter.ZeroAtFilter]
        simpa [h, g.zero.eq_of_nhds] using ((g.tendsto_scale v).const_sub 1).ofReal.mul tendsto_const_nhds
