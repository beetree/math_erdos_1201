...
public import Erdos1201.Vendor.Analysis.RotationSumLimitLaws.RamanujanSums
...
theorem sum_moebius_gcd (a q : ℕ) :
  ...
  change (if Nat.gcd a q = 1 then 1 else 0) = _
...
private theorem sum_phase_multiples_eq_complete {q d n : ℕ} (hq : q ≠ 0)
  ...
  · intro a ha
    ...
    rw [Finset.mem_range]
    exact (Nat.div_lt_div_right hd0 hdDvd).2 (Finset.mem_range.mp ha.1)
  · intro a₁ ha₁ a₂ ha₂ heq
...
theorem completeCharacterSumInt_eq {q : ℕ} (r : ℤ) (hq : q ≠ 0) :
  ...
  cases r with
  | ofNat n =>
      change completeCharacterSum q n = if q ∣ n then (q : ℂ) else 0
      exact completeCharacterSum_eq q n hq
  | negSucc n =>
  ...
  calc
    ...
    _ = ∑ a ∈ reducedResidues p, ∑ b ∈ reducedResidues p',
      ...
      · have hdvd := hiff.2 hsame
        rw [ite_eq_left hdvd, ite_eq_left hsame]
      · have hnotdvd : ¬L ∣ (crossNumerator p p' a b).natAbs := fun h ↦ hsame (hiff.1 h)
        rw [ite_eq_right hnotdvd, ite_eq_right hsame]
    _ = if p = p' then ((p * Nat.totient p : ℕ) : ℂ) else 0 := by
      by_cases hpp : p = p'
      · subst p'
        have hcard : (reducedResidues p).card = Nat.totient p := by
          simpa [reducedResidues, Nat.coprime_comm] using
            (Nat.totient_eq_card_coprime p).symm
        simp [L, hcard]
        ring
      · simp [hpp]
